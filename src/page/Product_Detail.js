...
useEffect(() => {
  ...
  return <div>
      <>
      <div ><div className="detail_title">상품 정보</div></div>
      <Container className="product_wrap">
        <Row>
          ...
          <div>
            ...
            <div>배송사 : CJ대한유통</div>
            <div style={{color:"rgb(224, 75, 75)"}}><strong>오픈 event</strong></div>
            <div>상품 구매시 무료배송 </div>
            <div>이벤트 기간 : 2023-04~2023-07</div>
            </div>
          </div>
          <div className="company_info">
            <h6 style={{color:"#395E97"}}> <strong>판매자 정보</strong> </h6>
          <div><strong>사업장 이름 </strong> : {my && my.company.name}</div>
          <div><strong>전화번호 </strong> : {my && my.phone}</div>
          <div><strong>이메일 </strong> : {my && my.company?.email}</div>
          <div><strong>사업장 주소 </strong> : {my  && my.company?.address.address}</div>
          </div>
          <div>
